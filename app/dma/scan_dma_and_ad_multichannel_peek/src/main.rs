...
use stm32f1xx_hal::pac::ADC1;
use stm32f1xx_hal::prelude::{_fugit_RateExtU32, _stm32_hal_adc_ChannelTimeSequence, _stm32_hal_dma_CircReadDma, _stm32_hal_dma_DmaExt, _stm32_hal_flash_FlashExt, _stm32_hal_gpio_GpioExt};
...
fn main() -> ! {
    ...
    oled::show_num(&mut scl, &mut sda, 1, 5, buf[1][0].into(), 2);
    oled::show_num(&mut scl, &mut sda, 2, 5, buf[2][0].into(), 2);
    oled::show_num(&mut scl, &mut sda, 3, 5, buf[3][0].into(), 2);
    oled::show_num(&mut scl, &mut sda, 4, 5, buf[4][0].into(), 2);

    oled::show_num(&mut scl, &mut sda, 1, 8, buf[1][1].into(), 2);
    oled::show_num(&mut scl, &mut sda, 2, 8, buf[2][1].into(), 2);
    oled::show_num(&mut scl, &mut sda, 3, 8, buf[3][1].into(), 2);
    oled::show_num(&mut scl, &mut sda, 4, 8, buf[4][1].into(), 2);
