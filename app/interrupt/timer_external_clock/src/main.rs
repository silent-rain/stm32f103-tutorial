...
fn main() -> ! {
    ...
    oled::show_string(&mut scl, &mut sda, 3, 1, "Psc:");
    oled::show_string(&mut scl, &mut sda, 4, 1, "Cnt:");
    loop {
        oled::show_num(&mut scl, &mut sda, 1, 5, get_num(), 5);
        oled::show_num(&mut scl, &mut sda, 2, 5, get_arr(), 5);
        oled::show_num(&mut scl, &mut sda, 3, 5, get_psc(), 5);
        oled::show_num(&mut scl, &mut sda, 4, 5, get_counter(), 5);
    }
}
